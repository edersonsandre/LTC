...
foreach ($tableDefinition['fields'] as $field) {
    ...
    if ($field['is_primary_key']) {
        ...
        if ($field['is_required']) {
            $fieldConfigs[] = '"required" => true';
        }

        //$fieldsConfigs[] = '->setAttrib("class", "element-input")';
    } else {
        $fieldConfigs[] = "'label' => '$field[label]'";
    ...
    $fieldCode = <<<ELEMENT
        \$this->addElement('$fieldType', '{$field['name']}', array(
                $fieldConfigs
            )
        );
ELEMENT;
 ...
 *
 */
class {$tableDefinition['formClassName']} extends Twitter_Bootstrap_Form_Horizontal
{
